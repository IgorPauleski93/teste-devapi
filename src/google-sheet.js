  ...
  let cleanContacts = await getSpreadsheet().then(sheet => {
    ...
    return sheet.getRows().then(rows => {
      rows.map(row => {
        ...
        if(hasEmptyField) {
          console.log(`O contato na linha "${row._rowNumber}" encontra-se o campo vazio. *Todos os campos são obrigatórios!*`);
        } else {
          if(validateContactEmail(contact.email)) {
            contacts.push(contact);
          } else {
            console.log(`O e-mail do contato na linha "${row._rowNumber}" não é corporativo`);
          }
        }
      });
      return contacts;
